...
def save_file(file_byte: bytes, 
    ...
    app_work.send_task("app_tasks.ocr_pdf_task.pdf_to_md", args=[data])
...
async def upload_file(background_tasks: BackgroundTasks,
                    ...
                    body:dict = Body({}, description="其他参数")):
    file_path = Path("tmp")/f'{str(uuid.uuid4())[:5]}-{file.filename}'
    file_byte = await file.read()    
    logger.info(f"收到OCR {file.filename}请求")
    background_tasks.add_task(save_file, 
                              file_path=file_path, 
                              file_byte=file_byte, 
                              user_id = user_id,
                              body = body)
    return {"file_path": str(file.filename),"body":body}
